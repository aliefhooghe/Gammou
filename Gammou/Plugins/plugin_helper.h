...
#define EXPORT_DEFAULT_FACTORY(sound_component_type, factory_name, description, factory_id) \
	DYNAMIC_EXPORT( \
		abstract_plugin_factory *make_factory() \
			{ return new default_plugin_factory<sound_component_type>(factory_name, description, factory_id);}) \
	DYNAMIC_EXPORT( \
