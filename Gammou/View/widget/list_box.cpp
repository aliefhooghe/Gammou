...
namespace Gammou {
	...
	namespace View {
		...
		list_box::list_box(const unsigned int x, const unsigned int y, const unsigned int width, const unsigned int height, 
			...
			const color background, const color font_color, const unsigned int font_size)
		...
		{
			if (m_items.size() != 0) {
				const int int_dist = static_cast<int>(distance);
				const int n = static_cast<int>(m_first_displayed) - int_dist;

				if (n < 0)
					m_first_displayed = 0;
				else if (n >= static_cast<int>(m_items.size()))
					m_first_displayed = static_cast<unsigned int>(m_items.size() - 1);
				else
					m_first_displayed = n;
